...
function lef_submit_reservation()
{
	...
	$property = $wpdb->get_row($wpdb->prepare(
		"SELECT title, host_id, price FROM {$wpdb->prefix}ls_property WHERE id = %d",
		$property_id
	));
		...
		case 'duplicate':
			...
			foreach ($valid_ids as $p_id) {
				...
				if ($prop) {
					...
					$clean_prop = array();
					foreach ($prop as $k => $v) {
						if (in_array($k, $columns)) $clean_prop[$k] = $v;
					}
					...
					if ($new_id) {
						...
						foreach ($images as $img) {
							...
							$clean_img = array();
							foreach ($img as $k => $v) {
								if (in_array($k, $img_cols)) $clean_img[$k] = $v;
							}
						...
						foreach ($dates as $date_row) {
							...
							$clean_date = array();
							foreach ($date_row as $k => $v) {
								if (in_array($k, $date_cols)) $clean_date[$k] = $v;
							}
