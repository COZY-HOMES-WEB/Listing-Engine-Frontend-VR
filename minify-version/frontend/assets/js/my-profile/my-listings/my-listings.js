  ...
  const LEF_HostListings = {
    ...
    bindEvents() {
      ...
      this.$tabs.on("click", (e) => {
        ...
        this.$tabs
          .removeClass("lef-host-list-tab-active")
          .attr("aria-pressed", "false");
        $(e.currentTarget)
          .addClass("lef-host-list-tab-active")
      ...
      this.$pageControls.on("click", "[data-lef-host-list-page]", (e) => {
        ...
        const totalPages = Math.max(
          1,
          Math.ceil(filtered.length / this.perPage),
        );
      ...
      this.$bulkApplyBtn.on("click", () => {
        ...
        if (window.LEF_Confirm) {
          window.LEF_Confirm.open(
            {
              title: "Change Status",
              message: "Change status of selected properties?",
            },
            (confirmed) => {
              if (confirmed)
                this.handleAction("change_status", ids, { status });
            },
          );
        }
      });
          ...
          window.LEF_Confirm.open(
            {
              ...
              message: "Delete selected properties? This cannot be undone.",
            },
            (confirmed) => {
    ...
    fetchData() {
      this.$cardList.html(
        '<div style="padding:40px; text-align:center;"><div class="lef-spinner" style="margin: 0 auto;"></div></div>',
      );
          ...
          if (response.success) {
            ...
            if (window.LEF_Toast)
              window.LEF_Toast.show(
                response.data.message || "Failed to load listings",
                "error",
              );
            this.$cardList.html(
    ...
    renderCards(pageItems, startIndex) {
      const html = pageItems
        .map((item, index) => {
          ...
          let fallbackUrl =
            window.lefMyProfileData && window.lefMyProfileData.plugin_url
              ? window.lefMyProfileData.plugin_url +
                "global-assets/images/placeholder.png"
              : "";
